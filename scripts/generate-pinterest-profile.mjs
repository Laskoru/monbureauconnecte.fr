...
const COVER_PHOTO_URL =
  'https://images.unsplash.com/photo-1623281185000-6940e5347d2e?q=80&w=1600&auto=format&fit=crop';
...
async function generateCoverPhoto() {
  const W = 1600;
  const H = 900;
  const res = await fetch(COVER_PHOTO_URL);
  if (!res.ok) throw new Error(`fetch couverture -> ${res.status}`);
